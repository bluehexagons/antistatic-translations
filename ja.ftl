...
controls-tag-delete-help = 確認後に {$tag} とその保存済み設定を削除します。
controls-tag-delete-title = タグを削除しますか？
controls-tag-delete-body = 「{$tag}」とその保存済み設定を削除しますか？
...
controls-current-menu-controls-summary =
    {$source}
    移動: {$navigate} · 決定: {$confirm} · 戻る: {$back}
...
controls-remap-protected-ui-bindings = メニュー操作を保つため、UI 決定と UI 戻るの割り当てを最低 1 つずつ残してください。
...
controls-remap-reset-help = このタグ付きプロファイルを既定のレイアウトに戻します。2 回の確認が必要です。
...
main-controls-help = コントローラータグを作成し、タグごとの操作割り当てを編集します。
